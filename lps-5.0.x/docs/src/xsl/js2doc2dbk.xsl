...
<!ENTITY cssstyle        '(doc/tag[@name="cssstyle"][last()]/text)'>
        ...
        <xsl:if test="count($visible-members) > 0">
          <informaltable frame="none" pgwide="1">
            <tgroup cols="5" colsep="0" rowsep="0">
              ...
              <thead>
                <row>
                  <entry align="left">Name (CSS property)</entry>
...
?>
  ...
  <xsl:template name="unique-attribute">
    ...
    <row rowsep="1">
      <xsl:choose>
        <xsl:when test="&cssstyle;">
          <entry role="attrcssstyle"><xsl:value-of select="&cssstyle;" /></entry>
        </xsl:when>
        <xsl:otherwise>
