  ...
  <xsl:template name="quote-canvas-parameter">
    ...
    <xsl:if test="$live">
      <div class="embedded-canvas">
        ...
        <xsl:variable name="swf-embed-params">{url: '<xsl:value-of select="concat($fname, '?lzt=swf&amp;lzr=swf10', $query-param)"/>', id: '<xsl:value-of select="concat($canvas-id,'SWF')"/>', history: false, cancelmousewheel: true <xsl:value-of select="$canvas-parameters"/>}</xsl:variable>
...
?>
    ...
    <xsl:if test="$live">
      ...
      <xsl:variable name="edit-href-dhtml">
        <xsl:call-template name="relative.path.to.lpshome" />
        <xsl:text>laszlo-explorer/editor.jsp?src=docs/</xsl:text>         
        <xsl:call-template name="base.book.name"/>
        <xsl:text>/</xsl:text>
        <xsl:value-of select="$fname"/>
        <xsl:text>&amp;lzr=dhtml</xsl:text>
      </xsl:variable>
      <xsl:variable name="editbuttonimg">
        <xsl:call-template name="relative.path.to.lpshome"/>
        <xsl:text>docs/includes/edit_button.png</xsl:text>
      </xsl:variable>
      <xsl:text>&#x0a;</xsl:text>
      <div style="position:relative;height:40px;top:7px;">
        <div class="edit-button" style="width:100px;height:25px;">
          <a href="{$edit-href}" target="_blank">
            <img src="{$editbuttonimg}" border="0" alt="edit" />
            <div style="position:relative;color:white;top:-22px;left:47px;width: 73px;">Flash</div>
          </a>
        </div>
        <div class="edit-button" style="width:100px;height:25px;position:relative;top:-25px;left:118px;">
          <a href="{$edit-href-dhtml}" target="_blank">
            <img src="{$editbuttonimg}" border="0" alt="edit" />
            <div style="position:relative;color:white;top:-22px;left:43px;width:73px;">HTML5</div>
          </a>
        </div>
      </div>
      <xsl:text>&#x0a;</xsl:text>
  ...
  <xsl:template match="programlisting[@role='lzx-embednew']">
    ...
    <xsl:variable name="text" select="code/text()"/>      
      ...
      <xsl:variable name="canvas-id" select="generate-id(.)"/>
      <xsl:variable name="swf-embed-params">{url: '<xsl:value-of select="concat($lzxdir, $fname, '?lzt=swf&amp;lzr=swf10', $query-param)"/>', id: '<xsl:value-of select="concat($canvas-id,'SWF')"/>', width: '<xsl:copy-of select="$canvas-width"/>', height: '<xsl:copy-of select="$canvas-height"/>', history: false}</xsl:variable>
