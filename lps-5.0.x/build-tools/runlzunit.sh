...
# allow javascript in Firefox to close windows by itself
    (cd ~/Library/Application\ Support/Firefox/Profiles/*; echo 'user_pref("dom.allow_scripts_to_close_windows", true);' > user.js)
    ;;
Linux)
    build_os=unix
    browser=firefox
...
# create the directory containing the log file as needed
...
build-tools/prepare-lzo-test.sh $runtime || exit 1
