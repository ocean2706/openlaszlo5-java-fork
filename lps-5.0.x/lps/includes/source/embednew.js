...
lz.embed = {
    ...
    ,_setSWF: function (url, app, swfargs, properties, minimumVersion) {
        ...
        if (embed.browser.isIE && url.indexOf('swf8') == -1 && ! options.cancelkeyboardcontrol) {
            ...
            document.onkeydown = function(e) {
                ...
                return (e.keyCode != 8);  // LPP-10002
            }
        }
        else if (embed.browser.isFirefox) {  // LPP-10002
     ...
     *
     * @param url:String url to LFC, e.g. '/trunk-clean/lps/includes/lfc/LFCdhtml-debug.js'
     * @param serverroot:String Base URL to load resources from, e.g. '/trunk-clean/' 
     */
    ...
    // Loads a JS library from the specified URL.  If callback arg is specified, it will be added to a queue of callbacks.
    ...
    __getqueryurl: function (url) {
        ...
        if (lzopts != null) {
            ...
            for (var lzopt in opts) {
                var sval = opts[lzopt];
                //  For back compatibility, coerce these known args to booleans
    ...
    _setCanvasAttributeDequeue: function () {
        while (this._setCanvasAttributeQ.length > 0) {
            var a = this._setCanvasAttributeQ.shift();
    ...
    _gotFocus: function() {
        ...
        setTimeout("lz.embed._broadcastMethod('_sendAllKeysUp')", 200);

        this.callMethod("lz.Focus.applicationFocus()");
    }
    ,/** @access private */   
    _gotBlur: function() {
        // Remove focus from the application
        this.callMethod("lz.Focus.applicationBlur()");
    }
    ,/** @access private */
    ...
    attachEventHandler: function(eventscope, eventname, callbackscope, callbackname, closure) {
        try {
            ...
            var handlers = lz.embed._handlers;
            var h = handlers[s];
            if (h != null) {
    ...
    getAbsolutePosition: function(el) {
        ...
        if (el !== document.body && el.getBoundingClientRect ) {
            if (! el.parentNode) {
            ...
            //alert(([box.left, box.top, scrollLeft, scrollTop, clientLeft, clientTop]).join(', '));
            return {x: Math.floor(box.left + scrollLeft - clientLeft), y: Math.floor(box.top + scrollTop - clientTop)};
...
lz.embed.attachEventHandler(window, 'blur', lz.embed, '_gotBlur');
