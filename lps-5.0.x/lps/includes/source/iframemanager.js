...
lz.embed.iframemanager = {
    ...
    ,setPosition: function(id, x, y, width, height, visible, z, flash_width) { 
        ...
        if (div) {
            ...
            if (y != null && ! isNaN(y)) div.style.top = (y + pos.y) + 'px';
    ...
    ,__gotload: function(id) {
        ...
        //LPP-10363, set events when the iframe is loaded compeletely for ie in dhtml runtime.
        if ($dhtml && lz.embed.browser.isIE) {
             if (iframe.__owner['__mouseevents']) this.setSendMouseEvents(id, iframe.__owner.__mouseevents);
        }
...
//        }
    ...
    ,__getRPCMethods: function(id) {
        var iframemanager = lz.embed.iframemanager,
            iframe = iframemanager.getFrameWindow(id);
        var callback = function(returnObj) {
            var matches = [];
            for (var i = 0, l = returnObj.length; i < l; i++) {
                var el = returnObj[i];
                if (el.destination === iframe) {
                    matches.push(el.publicProcedureName);
                }
            }
            var str = JSON.stringify(matches);
            //console.log('found __rpcmethods', str);
            iframemanager.asyncCallback(id,'__rpcmethods',str);
        }
        pmrpc.discover({callback: callback}); 
    }
