...
/* @public */
LzHTTPLoader.prototype.setCredentialled = function (cred) {
    this.credentialled = cred;
}
...
LzHTTPLoader.prototype.setupTimeout = function (loader, duration) {
    loader.__timeoutID = setTimeoutOneArg(LzHTTPLoader.__LZhandleXMLHTTPTimeout, duration, loader);
}
...
LzHTTPLoader.prototype.loadXMLDoc = function (method, url, headers, postbody, ignorewhite) {
    if (this.req) {
                ...
                if (self.__timeout) {
                    ...
                    if (status == 200 || status == 304) {
                        ...
                        if ($debug && self.iscors && self.responseStatus == 0) {
                            var regExp = new RegExp( "\/\/(www\.)?(.[^/:]+)" ),
                            dataDomain = self.requesturl.match( regExp )[2];
                            Debug.warn("Cross-domain data request to "+ dataDomain + " with URL " + self.requesturl + " failed. " +
        ...
        // Allow credentials for CORS request
        if (this.iscors && this.credentialled &&
            this.req.withCredentials !== undefined) {
...
LzHTTPLoader.prototype.checkIfCORS = function () {
    /* Request is a not a CORS request if it's
         1. proxied
         2. if the host and port property of the lz.URL object are null. The
            request is relative to the application base URL, and therefore
            cannot be cross-origin.
    */
    var url = new lz.URL(this.requesturl);
    if ( this.options.proxied === "true" || ( url.host === null && url.port === null) ) {
            return false;
    }
    var domainRegExp = new RegExp("\/\/(www\.)?(.[^/:]+)"),
        ...
        port = url.port;
    if (port == null) {
        // No port information in requesturl, take the default values
        if (isHttps) {
            port = lz.Browser.defaultPortNums.https;
        } else {
            port = lz.Browser.defaultPortNums.http;
        }
    }
    this.iscors = (url.host != document.location.hostname) || (document.location.port != port) ;
    return this.iscors;
}
