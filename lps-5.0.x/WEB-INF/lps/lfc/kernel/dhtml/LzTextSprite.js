 ...
 */
LzTextSprite.prototype.setDirection  = function (dir) {
    this.scrolldiv.style.direction = dir.toUpperCase();
}
...
LzTextSprite.prototype.__updatefieldsize = function (){
  ...
  this.__updatefieldsizeTID = setTimeoutNoArgs(cstr, 0);
}
...
LzTextSprite.prototype.getTextWidth = function (force) {
  ...
  if (width != 0 && this.quirks['fix_ie9_textwidth']) {
      // IE9 measurements are off; text strings wrap without this padding
      width += 1;
  }
...
LzTextSprite.prototype.getTextDimension = function (dimension) {
  ...
  switch (dimension) {
    case 'lineheight':
      ...
      break;
    case 'height':
       // LPP-10212. Add a trailing character if the string ends in new line
      if (string.length > 0 && string.charAt(string.length-1) == '\n') {
          string = string + "a";
      }
       width = this.scrolldiv.style.width;
      break;
    case 'width':
      if (this.text == '') { return 0; }
      break;
  ...
  var style = ("padding:0px;overflow:visible;width:" + width + ";height:auto;" +
           ...
           ((this._whiteSpace) ? ("white-space:" + ( dimension == "width" ? "pre": this._whiteSpace ) + ";") : ""));
...
LzTextSprite.prototype.setWidth = function (w){
    ...
    // the way
    if (this.scrolling) {
       if (  (this instanceof LzInputTextSprite ) 
                && (lz.embed.browser.isFirefox ) )  {
        // LPP-10162 In Firefox,
          scrolldivwidth += 3 ;
       }   
...
// LPP-10191. IE9 needs help to resize the text to the current size. This
// is handled in a callback method which resets the lineheight setting of
// the div.
LzTextSprite.prototype.__lineheight = '';
LzTextSprite.prototype.__updatewhitespace = function (){
...
LzTextSprite.prototype.setHeight = function (h) {
    ...
    if (hp !== this.__heightcss) {
      ...
      var scrolldiv = this.scrolldiv;
      this.__lineheight = scrolldiv.style.lineHeight;
      scrolldiv.style.height = hp;

      if (this.quirks.fix_ie_divheight_change) {
        scrolldiv.style.lineHeight = 'inherit';
        var callback = lz.BrowserUtils.getcallbackfunc(this, '__updatewhitespace', []);
