...
public class LzTextSprite extends LzSprite {
    #passthrough (toplevel:true) {
        ...
        public static const PAD_MULTI_TEXTHEIGHT:Number = 0;
                    ...
                    if (type == MouseEvent.MOUSE_OVER || type == MouseEvent.MOUSE_OUT) {
                        ...
                        if ((relObj is TextField || relObj is LzTLFTextField) && relObj.parent is LzTextSprite) {
                            ...
                            if (lztext.forwardsMouse) {
                                relObj = lztext.getNextMouseObject(e) cast InteractiveObject;
                            }
                        }
                        if (relObj === obj) {
                            forward = false;
...
#passthrough  {
        ...
        var nakedCrRegExp = new RegExp("\\r(.)", "mg");
        var crLfRegExp = new RegExp("\\r\\n", "mg");
        var crRegExp = new RegExp("\\r", "mg");

/* code to support hexDumpString() utility fcn:
        ...
        public function toHex(c: String): String {
        ...
        public function hexDumpString(s:String) :String {
            var hexes = [];
            for (var i=0; i<s.length; i++) {
                hexes.push(this.toHex(s.charAt(i)));
            }
            return hexes.join(" ");
        }
*/
        ...
        public function textInputHandler(e:TextEvent) :void {
            // LPP-10003
            // If there's a \r not immediately next to \n, or one at the end,
            // we'll handle the paste right here.  Otherwise, during the paste,
            // the \r is effectively stripped out of textfield.text.
            var pasted = e.text;
            if (pasted.search(nakedCrRegExp) != -1 ||
                (pasted.length > 0 && pasted.charAt(length-1) == "\r")) {
              try {
                // \r\n combination are converted to simple \n,
                // we don't want this to result in extra lines.
                var newtext = pasted.replace(crLfRegExp, '\n');

                // Now we can just change all \r to \n
                newtext = newtext.replace(crRegExp, '\n');

                //Debug.write('adjusted pasted text', hexDumpString(pasted), hexDumpString(newtext));
                textfield.replaceSelectedText(newtext);
                e.preventDefault();
              }
              catch (e) {
                // replaceSelectedText can throw an error if there is a style
                // sheet.  In that case (or any other error), 'preventDefault'
                // won't be called, and the normal paste mechanism will
                // be used.
              }
            }
        }
        ...
        override public function setHeight( h:Number ):void {
            if (owner.hassetheight) {
              ...
              // whole line, to prevent the textfield clipping (which
              // causes undesirable artifacts), while preserving the
              // scroll dimensions.
              textfield.height = Math.ceil(h / lineheight) * lineheight + (textfield.multiline ? PAD_MULTI_TEXTHEIGHT : PAD_TEXTHEIGHT);
            } else {
              setClip(false);
              super.setHeight(h);
              textfield.height = h;
        ...
        public function __initTextProperties (args:Object) :void {
            ...
            if (! args.hassetheight) {
                ...
                // See setHeight
                setClip(true);
                textfield.height = Math.ceil(args.height / lineheight) * lineheight + (textfield.multiline ? PAD_MULTI_TEXTHEIGHT : PAD_TEXTHEIGHT);
            }
            ...
            if (! args.hassetheight) {
                ...
                h += (textfield.multiline ? PAD_MULTI_TEXTHEIGHT : PAD_TEXTHEIGHT); //2*2px gutter, see flash docs for flash.text.TextLineMetrics 
                this.setHeight(h);
            }
        ...
        */
        ...
        public function setText ( t:String ):void {
            ...
            if (! owner.hassetheight) {
                ...
                if (theight == 0) {
                    this.__setFormat(); // Compute lineheight
                    theight = this.lineheight;
                    if (this.multiline) theight *= textfield.numLines;
                }
                // I rearranged these line to be closer to what happens
                // in __initTextProperties.
                // Only PAD_TEXTHEIGHT is needed. I'm not sure why
                // LPP-10001, LPP-10041
        ...
        public function __setFormat ():void {
            // Can't do anything until we have at least a font and a
            // size
            if (this.fontname == null || this.fontsize == 0) { return; }
            this.setFontInfo();
        ...
        public function getTextfieldHeight (force=null) :Number {
            ...
            var tsv:Number = textfield.scrollV;
            var tsh:Number = textfield.scrollH;
            ...
            if (h == (textfield.multiline ? LzTextSprite.PAD_MULTI_TEXTHEIGHT : LzTextSprite.PAD_TEXTHEIGHT)) {
            ...
            textfield.scrollV = tsv;
            textfield.scrollH = tsh;
...
public class LzTextField extends TextField {
  #passthrough (toplevel:true) {
    import flash.text.TextField;
  }#
  ...
  override public function get scrollH ():int {
    Debug.info("%w.get scrollH => %w", (this.parent as LzSprite).owner, super.scrollH);
    return super.scrollH;
  };

  override public function set scrollH (value:int):void {
    Debug.info("%w.set scrollH %w => %w", (this.parent as LzSprite).owner, super.scrollH, value);
    super.scrollH = value;
  }

  override public function setSelection(beginIndex:int, endIndex:int):void {
    Debug.info("%w.setSelection(%w, %w)", (this.parent as LzSprite).owner, beginIndex, endIndex);
