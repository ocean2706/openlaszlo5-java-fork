...
public class ClientSOAPService 
{
    ...
    public String createObjectProgram(LZSOAPService service) {
        ...
        //------------------------------------------------------------
        Map<String, LZSOAPOperation> operations = service.getOperations();
        for (String opName : operations.keySet()) {
            LZSOAPOperation op = operations.get(opName);
    ...
    void pushParts(LZSOAPMessage inMesg, String style) {
        ...
        body.append("[");
        List<LZSOAPPart> parts = inMesg.getParts();
        int item = 0;
     ...
     */
    public  void pushComplexTypeInfo(Map<QName, ComplexType> ctm) {
        if (ctm == null) {
            body.append("null");
            return;
        }

        Iterator<Map.Entry<QName, ComplexType>> iter = ctm.entrySet().iterator();
        if (! iter.hasNext()) {
            body.append("null");
            return;
        }
        ...
        int count  = 0;
        while (iter.hasNext()) {
            Map.Entry<QName, ComplexType> entry = iter.next();
     ...
     */
     void pushMembers(ComplexType ct) {
        Map<String, QName> members = ct.getMembers();
        if (members == null) {
            body.append("null");
            return;
        }

        Iterator<Map.Entry<String, QName>> iter = members.entrySet().iterator();
        if (! iter.hasNext()) {
            body.append("null");
            return;
        }
