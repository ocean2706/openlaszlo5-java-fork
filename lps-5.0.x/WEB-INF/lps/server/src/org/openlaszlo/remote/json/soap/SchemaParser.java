...
public class SchemaParser
{
    ...
    **/
    ...
    Map<QName, Vector<ComplexType>> mBaseForwardTypeMap;
    Map<QName, Vector<ComplexType>> mArrayForwardTypeMap;

    Map<QName, ComplexType> mComplexTypeMap;
    ...
    public SchemaParser(WSDLParser wp, Element schema) {
        ...
        mBaseForwardTypeMap = new HashMap<QName, Vector<ComplexType>>();
        mArrayForwardTypeMap = new HashMap<QName, Vector<ComplexType>>();
     ...
     */
    public void parse(Map<QName, ComplexType> complexTypeMap) { 
        ...
        if (mBaseForwardTypeMap.size() != 0) {
            ...
            for (QName baseQName : mBaseForwardTypeMap.keySet()) {
    ...
    void fixBaseForwardRefs(ComplexType baseType) {
        ...
        Vector<ComplexType> list = (Vector<ComplexType>)mBaseForwardTypeMap.get(baseQName);
        if (list == null) return;
        for (int i=0; i < list.size(); i++) {
            ComplexType ct = list.get(i);
            ct.setBase(baseType);
        }
    ...
    void fixArrayForwardRefs(ComplexType arrayItemType) {
        ...
        Vector<ComplexType> list = mArrayForwardTypeMap.get(arrayQName);
        if (list == null) return;
        for (int i=0; i < list.size(); i++) {
            ComplexType ct = list.get(i);
    ...
    void setBaseType(QName baseQName, ComplexType ct) {
        ComplexType baseType = mComplexTypeMap.get(baseQName);
        if (baseType == null) {
            ...
            if (Constants.isSOAP_ENC(baseQName.getNamespaceURI()) ) {
                ...
                Vector<ComplexType> list = mBaseForwardTypeMap.get(baseQName);
                if (list == null) { 
                    list = new Vector<ComplexType>();
    ...
    void setArrayItemType(QName arrayQName, ComplexType ct) {
        ComplexType arrayItemType = mComplexTypeMap.get(arrayQName);
    ...
    ComplexType checkAllOrSequence(String name, Element node) throws Exception {
        ...
        Map<String, QName> members = new HashMap<String, QName>();
