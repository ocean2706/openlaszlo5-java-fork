...
public class LZSOAPService
{
     ...
     * swf7, etc.) */
    Hashtable<String, byte[]> mClientSOAPServiceMap = new Hashtable<String, byte[]>();

    /** Map of LZSOAPOperations. */
    Map<String, LZSOAPOperation> mOperations = null;

    /** Map of schema complex types. */
    Map<QName, ComplexType> mSchemaComplexTypes = null;
    ...
    public Object[] invoke(String operation, String xml) 
        ...
        try {
            ...
            LZSOAPOperation op = mOperations.get(operation);
     ...
     */
    public List<Object> setRPCParams(Call call, NodeList paramsList, List<LZSOAPPart> parts, 
                             LZSOAPOperation op)
        ...
        for (int i=0; i < paramsList.getLength(); i++) {
            ...
            Node node = paramsList.item(i);
            if (node.getNodeType() != Node.ELEMENT_NODE) continue;

            Element p = (Element)node;
            Node paramNode = getParamValue(p);

            LZSOAPPart part = parts.get(pos);
 ...
 */
                        org.openlaszlo.i18n.LaszloMessages.getMessage(
                                LZSOAPService.class.getName(),"051018-304", new Object[] {pos + 1})
                    );
                }
                ...
                ArrayList<ArrayWrapper> list = new ArrayList<ArrayWrapper>();
                list.add(aw);
                params.add(list);
                if (mLogger.isDebugEnabled()) {
                ...
                if (isTextNode(paramNode)) { // primitive param
                    ...
                    params.add(parts.get(pos).valueOf(strValue));
                    if (mLogger.isDebugEnabled()) {
                        mLogger.debug(
/* (non-Javadoc)
 * @i18n.test
 ...
 */
                        org.openlaszlo.i18n.LaszloMessages.getMessage(
                                LZSOAPService.class.getName(),"051018-353", new Object[] {pos + 1, paramNode})
                        );
                }
 ...
 */
     ...
     */
    public List<Object> setDocumentParams(Call call, NodeList paramsList, List<LZSOAPPart> parts, 
                                  LZSOAPOperation op)
        throws ServiceException, IOException, SAXException, DOMException {
 ...
 */
        ...
        for (int i=0; i < paramsList.getLength(); i++) {
            ...
            String data = URLDecoder.decode(text.getData(), "UTF-8");
 ...
 */
        ...
        try {
            ...
            List<LZSOAPPart> parts = op.getInputMessage().getParts();
    ...
    public void setOperations(Map<String, LZSOAPOperation> operations) {
        mOperations = operations;
    }

    /** @return map of LZSOAPOperations. */
    public Map<String, LZSOAPOperation> getOperations() {
        return mOperations;
    }
     ...
     */
    public void setSchemaComplexTypes(Map<QName, ComplexType> schemaComplexTypes) {
        mSchemaComplexTypes = schemaComplexTypes;

        // now register these complex types with type mapper.
        if (mSchemaComplexTypes != null) {
            for (ComplexType value : mSchemaComplexTypes.values()) {
 ...
 */
     ...
     */
    public Map<QName, ComplexType> getSchemaComplexTypes() {
    ...
    public void toComplexTypesXML(StringBuffer sb) {
        ...
        if (mSchemaComplexTypes != null) {
            for (Map.Entry<QName, ComplexType> entry : mSchemaComplexTypes.entrySet()) {
                ComplexType ct = entry.getValue();
                ct.toXML(sb);
            }
        }
        sb.append("</complex-types>");
    ...
    public void toOperationXML(StringBuffer sb) {
        ...
        for (String key : mOperations.keySet()) {
            LZSOAPOperation op = mOperations.get(key);
            op.toXML(sb);
        }
        sb.append("</operations>");
    }
