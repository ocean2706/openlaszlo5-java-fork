...
public class XMLRPCJSONCompiler
{
     ...
     */
    static List<Element> getElementsByTagName(Element parent, String tag)
    {
    ...
    void writeArray(Element array)
        ...
        body.append("[");
        List<Element> data = getElementsByTagName(array, "data");
        if (data.size() != 1)
            throw new IOException("Invalid number of data elements in array");

        List<Element> values = getElementsByTagName(data.get(0), "value");
    ...
    void writeStruct(Element struct)
        ...
        body.append("{");
        List<Element> members = getElementsByTagName(struct, "member");
        // Push member values in reverse order
        for (int i = members.size()-1; i >= 0; --i) {
            writeMember(members.get(i));
            if (i > 0) {
                body.append(", ");
            }
        }
