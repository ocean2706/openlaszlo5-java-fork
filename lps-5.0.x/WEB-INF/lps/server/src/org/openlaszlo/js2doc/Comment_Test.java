...
* Copyright 2006-2007, 2011 Laszlo Systems, Inc.  All Rights Reserved.        *
...
public class Comment_Test extends XMLTestCase {
    ...
    public void testCommentScanner () {
        ...
        for (Iterator<String> iter = Arrays.asList(tests).iterator(); iter.hasNext();) {
            ...
            ArrayList<String> matches = Comment.extractRawComments(normalizedSource);

            assertTrue(source, iter.hasNext());
            int expectedMatches = Integer.parseInt(iter.next());

            assertEquals(source, expectedMatches, matches.size());

            final int n = matches.size();
            for (int i=0; i<n; i++) {
                String actual = matches.get(i);
                assertTrue(source, iter.hasNext());
                String expect = iter.next();
    ...
    public void testjs2docExtractFieldNames () {
        ...
        for (Iterator<String> iter = Arrays.asList(tests).iterator(); iter.hasNext();) {
            ...
            int expectedFields = Integer.parseInt(iter.next());
    ...
    public void testExtractAsXML() {
        ...
        for (Iterator<String> iter = Arrays.asList(tests).iterator(); iter.hasNext();) {
            ...
            try {
                ...
                assertXMLIdentical("JS2Doc.toXML(\"" + source + "\")", diff, true);
