...
public class SWF9External {
  ...
  private static Logger mLogger  = Logger.getLogger(SWF9External.class);
   ...
   */
  private HashMap<String, List<UniqueGlobalName>> uniqueFileNames = new HashMap<String, List<UniqueGlobalName>>();
  ...
  void writeOptionsFile() {
    ...
    private String optionsToString() {
      ...
      result.append("{");
      TreeMap<String, Object> sorted = new TreeMap<String, Object>(options);
          ...
          if (options.getBoolean(Compiler.INCREMENTAL_COMPILE)) {
            ...
            if (compilerOptionsChanged()) {
              if (mLogger.isDebugEnabled()) {
                mLogger.debug("swf9 compiler options changes, cleaning working dir");
              }
              deleteDirectoryFiles(workdir);
            }
          } else {
              if (mLogger.isDebugEnabled()) {
                mLogger.debug("cleaning working dir: "+workdir);
              }
            deleteDirectoryFiles(workdir);
          }
        }
      ...
      if (dir.isDirectory()) {
        ...
        for (int i=0; i<children.length; i++) {
          ...
          if (f.isFile() && cpath.endsWith(".as") && !mWrittenClassFiles.contains(cpath)) {
              if (mLogger.isDebugEnabled()) {
                mLogger.debug("removing unknown .as file "+f);
              }
            f.delete();
          }
        }
  ...
  public static File createCompilationWorkDir(String compileType, ScriptCompilerInfo scInfo)
    ...
    /**
     * Close a stream unconditionally.
     */
    private static void closeit(Closeable c)
    {
      try {
        ...
        mLogger.debug("Exception closing: " + ioe);
      }
    }
    ...
    public class ExternalCompilerErrorCollector extends OutputCollector {
      ...
      private List<ExternalCompilerError> errors = new ArrayList<ExternalCompilerError>();
      private ExternalCompilerError lastError = null;
      private TranslationUnit[] tunits;
      private List<String> severe = new ArrayList<String>();

      // we don't expect this to be terribly big, can fit in memory
      StringBuffer sb = new StringBuffer();

      public ExternalCompilerErrorCollector(InputStream is, List<TranslationUnit> tunits) {
        super(is);
        this.tunits = tunits.toArray(new TranslationUnit[0]);
      ...
      public void collect(String str) {
        ...
        if (matcher.find()) {
          ...
          if (lastError == null) {
            mLogger.debug("Stray error string from external compiler: " + str);
      ...
      public List<ExternalCompilerError> getErrors() {
        return errors;
      }

      public List<String> getSevereErrors() {
        return severe;
      }
    }
     ...
     */
    public String prettyCommand(List<String> cmd)
    {
      String cmdstr = "";
      for (String arg : cmd) {
        if (cmdstr.length() > 0)
     ...
     */
    public static void copyEnvVar(List<String> envvars, String varname) {
     ...
     */
    public void execCompileCommand(List<String> cmd, String dir, List<TranslationUnit> tunits,
                                   String outfileName)
      throws IOException          // TODO: [2007-11-20 dda] clean up, why catch only some exceptions?
    {
      @SuppressWarnings("unused")
      String compilerClass = cmd.remove(0);

      String[] cmdstr = cmd.toArray(new String[0]);
      String prettycmd = prettyCommand(cmd);
      if (mLogger.isDebugEnabled()) {
        mLogger.debug("Executing compiler: (cd " + dir + "; " + prettycmd + ")");
      }
      ...
      try {
        ...
        if (outcollect.getException() != null) {
          mLogger.debug("Error collecting compiler output: " + outcollect.getException());
          // TODO: [2007-11-20 dda] log this
        }
        String compilerOutput = outcollect.getOutput();
        if (compilerOutput.length() > 0) {
          mLogger.debug("compiler output:\n" + compilerOutput);
        }

        if (errcollect.getException() != null) {
          mLogger.debug("Error collecting compiler output: " + errcollect.getException());
          // TODO: [2007-11-20 dda] log this
        }
        List<String> severe = errcollect.getSevereErrors();
        if (severe.size() > 0) {
          for (String s : severe) {
            String errstr = "SEVERE ERROR: " + s;
            bigErrorString.add(errstr);
            mLogger.debug(errstr);
          }
        }
        List<ExternalCompilerError> errs = errcollect.getErrors();
        if (errs.size() > 0) {
          mLogger.debug("ERRORS: ");
          for (ExternalCompilerError err : errs) {
            ...
            mLogger.debug(actualSrcLine + srcLineStr + err.getErrorString());
        ...
        if (exitval != 0) {
          mLogger.debug("FAIL: compiler returned " + exitval);
        }
      }
      catch (InterruptedException ie) {
        throw new CompilerError("Interrupted compiler");
      }
      mLogger.debug("Done executing compiler");
      if (!new File(outfileName).exists()) {
        mLogger.debug("Intermediate file " + outfileName + ": does not exist");
     ...
     *        compiler, or if any part of the compilation process has problems
     */
    public void callJavaCompileCommand(List<String> acmd, String dir, List<TranslationUnit> tunits,
                                       String outfileName)
      throws IOException          // TODO: [2007-11-20 dda] clean up, why catch only some exceptions?
    {
      final List<String> cmd = acmd;
      final String compilerClass = cmd.remove(0);
      String prettycmd = prettyCommand(cmd);
      mLogger.debug("Executing compiler: (cd " + dir + "; " + prettycmd + ")");
      BigErrorString bigErrorString = new BigErrorString();
      ...
      if (options.getBoolean(Compiler.EMIT_AS3_ONLY)) {
        ...
        outf.close();
        mLogger.debug("option EMIT_AS3_ONLY set, returning without invoking flex compiler, call 'lcompile #' to compile as3");
        return;
      }
      ...
      Thread worker = new Thread() {
          ...
          public void run() {
            ...
            if (compilerClass.equals("mxmlc")) {
              ...
              exitval[0] = flex2.compiler.util.ThreadLocalToolkit.errorCount();
      ...
      try {
        ...
        if (exitval[0] != 0) {
          mLogger.debug("FAIL: compiler returned " + exitval[0]);
        }
      } catch (InterruptedException ie) {
        throw new CompilerError("Interrupted compiler");
      }
    ...
    public byte[] compileTranslationUnits(List<TranslationUnit> tunits, String compileType)
      throws IOException
    {
      return compileTranslationUnits(tunits, compileType, null, new HashSet<String>());
    }

    /**
     * Compile the given translation units, producing a binary output.
     */
  public byte[] compileTranslationUnits(List<TranslationUnit> tunits, String compileType, Set<File> swclibs, Set<String> externalClassNames)
      throws IOException
    {
      ...
      // Decode string values of form
      // version=10.1
      // air
      String fopts = (String)options.get(Compiler.FLEX_OPTIONS);
      String[] flex_options = null;
      if (fopts != null) {
        flex_options = fopts.split(",");
      }
      boolean air = false;
      String flex_version = LPS.getFlexVersionDefault();
      // low level option to force flex compiler -debug flag, even if laszlo app does not have debugging enabled
      boolean flexdebug = false;
      /// HOW TO FIND optional options??? what is syntax?
      // version=10.1,target=air,debug=true
      if (flex_options != null) {
        for (int i = 0; i < flex_options.length; i++) {
          String val = flex_options[i];
          if (val.startsWith("version=")) {
            flex_version = val.substring("version=".length());
          } else if (val.equals("air")) {
            air = true;
      ...
      if (air) {
        cmd.add("-load-config="+getFlexPathname("frameworks/air-config.xml"));
      } else {
      ...
      if (compileType == Config.LFCLIB ||
        ...
        // Look for compiler/platform/flex support classes
        String path = "/lfc/compiler/platform/flex".replace('/', File.separatorChar);
        cmd.add("-compiler.source-path+=" + new File(LPS.ROOT()+path).getCanonicalPath());
      }
      if (nameFunctions || flexdebug) {
      ...
      if (compileType == Config.LFCLIB ||
        ...
        cmd.add("-externs");
        for (String cname : externalClassNames) {
          cmd.add(cname);
        }
        ...
        cmd.add((String) options.get(Compiler.CANVAS_WIDTH, "800"));
        cmd.add((String) options.get(Compiler.CANVAS_HEIGHT, "600"));
        ...
        cmd.add("-includes");
        for (String varname : org.openlaszlo.compiler.Compiler.GLOBAL_RUNTIME_VARS) {
          cmd.add(varname);
        }
      }
      ...
      if (! options.getBoolean(Compiler.INCREMENTAL_COMPILE) &&  outf.exists()) {
        outf.delete();
      }
      ...
      //mLogger.debug("elapsed time in .as file writing: "+(mElapsed/1000)+" msec");
      //mLogger.debug("elapsed time in flex compilation: "+(System.currentTimeMillis() - mFlexTime)+" msec");
    ...
    private String getFileNameForClassName(String name, boolean isClass) {
      ...
      List<UniqueGlobalName> list = uniqueFileNames.get(lower);

      if (list == null) {
        list = new ArrayList<UniqueGlobalName>();
        uniqueFileNames.put(lower, list);
      } else {
        for (UniqueGlobalName unique : list) {
  ...
  HashSet<String> mWrittenClassFiles = new HashSet<String>();
    ...
    public void writeFile(TranslationUnit tunit, String pre, String post) {
      ...
      if (options.getBoolean(Compiler.INCREMENTAL_COMPILE)) {
        if (!diskfile.getName().equals("LzApplication.as") && diskfile.exists()) {
          ...
          if (classModified >= lastWritten) {
            //System.err.print("!");
            //mLogger.debug(" MODIFIED: "+tunit.getLZXFilename()+" asfile: "+tunit.getSourceFileName() + ":: "+new Date(lastWritten)+", lzxfile: "+new Date(classModified)+"\n");
          } else {
            // File was unmodified
          ...
          //mLogger.debug("diskfile for tunit "+tunit+" does not exist: "+diskfile);
        }
      }
      ...
      try {
        ...
        mLogger.debug("Exception in postprocessing, file=" + infilename + ": " + ioe);
