...
import org.openlaszlo.sc.Instructions.BranchInstruction;
...
public class Assembler implements Emitter {
  Hashtable<Object, Label> labels;
  ByteBuffer bytes;
  private static byte[] backingStore;
  Hashtable<String, Integer> constants;
  ...
  public static class Label {
    Object name;
    int location;
    List<LabelReference> references;
    ...
    public Label(Object name) {
      this.name = name;
      this.location = -1;
      this.references = new ArrayList<LabelReference>();
    }
    ...
    public void setLocation(ByteBuffer bytes) {
      ...
      for (LabelReference lr : references) {
  ...
  public Assembler() {
    this.labels = new Hashtable<Object, Label>(); // {String -> Label}
    ...
    this.constants = new Hashtable<String, Integer>(); // {String -> int}
  }
  ...
  public byte[] assemble(List<Instruction> instrs) {
    ...
    List<Label> unresolvedLabels = new ArrayList<Label>();
    // One wonders why this couldn't be an Iterator
    for (Enumeration<Label> i = labels.elements(); i.hasMoreElements(); ) {
      Label label = i.nextElement();
      if (! label.isResolved()) {
  ...
  public void emit(Instruction instr) {
    ...
    if (instr instanceof ConcreteInstruction && ((ConcreteInstruction)instr).op == Actions.CONSTANTS) {
      ...
      this.constants = new Hashtable<String, Integer>();
      for (ListIterator<Object> i = ((ConcreteInstruction)instr).args.listIterator(); i.hasNext(); ) {
        int index = i.nextIndex();
        constants.put((String) i.next(), index);
      }
