...
public class GenericVisitor implements ASTVisitor {
  ...
  ParseResult parseFile(File file, String userfname, String source) {
    if (Compiler.CachedParses == null) {
      Compiler.CachedParses = new ScriptCompilerCache<String, String, ParseResult>();
    }
    ...
    if (enableScriptCache) {
      entry = Compiler.CachedParses.get(sourceKey, sourceChecksum);
    }
    if ((entry == null)
  ...
  public SimpleNode visitStatement(SimpleNode node, SimpleNode[] children) {
    ...
    if (node instanceof ASTClassDefinition) {
      ...
      return visitForEachStatement(node, children);
    } else if (node instanceof ASTForEachVarStatement) {
      return visitForEachVarStatement(node, children);
      ...
      return visitDebuggerStatement(node, children);
