...
import org.openlaszlo.compiler.Main;
import org.openlaszlo.sc.GenericVisitor.ParseResult;
...
public class Compiler {
  ...
  public static ScriptCompilerCache<String, String, ParseResult> CachedParses;
  ...
  // work there).
  public static ScriptCompilerCache<String, String, Object> CachedInstructions;
  ...
  // Compiler Facade
  //
  public Compiler (Map<String, Object> initialOptions) {
    this.options = new OptionMap(initialOptions);
    if (! options.containsKey(COMPILE_TIME_CONSTANTS)) {
      options.put(COMPILE_TIME_CONSTANTS, new HashMap<String, Object>());
    }
  ...
  public static class OptionMap extends HashMap<String, Object> {
    public OptionMap () {
      super();
    }

    public OptionMap (Map<? extends String, ? extends Object> m) {
      super(m);
    }

    public OptionMap (List<List<Object>> pairs) {
      for (List<Object> pair : pairs) {
        put((String) pair.get(0), pair.get(1));
      }
    }
...
//       xformer.setOptions(options);
      ...
      if (isScript) {
        ...
        List<TranslationUnit> tunits = cg.makeTranslationUnits(translated, compress, obfuscate);
        translated = null;
        ...
        List<Instruction> instrs = ((CodeGenerator)cg).getCollector().getInstructions(true);
  ...
  public static String ERROR_WITH_THIS = "errorWithThis";
  ...
  public static String FLEX_OPTIONS = "flexoptions";
  ...
  public static String REFERENCE_CLASS = "referenceClass";
  public static String REGISTER_REFERENCE_CLASSES = "registerReferenceClasses";
  ...
  public static class Parser {
    private static final ParserVisitor NEW_INSTANCE = new NewInstanceVisitor();
    ...
    private void fold(SimpleNode node, int arity) {
      ...
      int size = node.size();
      if (size > arity) {
        SimpleNode child = (SimpleNode) node.jjtAccept(NEW_INSTANCE, null);
...
//         return node
    ...
    private SimpleNode visit(SimpleNode node, Map<String, Object> keys) {
      ...
      node.setChildren(result.toArray(children));
      return node;
    }

    private String fileLineInputString(SimpleNode n) {
      if (n == null || n.getFilename() == null || n.getLineNumber() < 0) {
    ...
    public SimpleNode substitute(SimpleNode nearnode, String str, Map<String, Object> keys) {
    ...
    public SimpleNode[] substituteStmts(SimpleNode nearnode, String str, Map<String, Object> keys) {
      SimpleNode fexpr = substitute(nearnode, "(function () {" + str + "})()", keys);
      return fexpr.get(0).get(1).getChildren();
    }

    public SimpleNode substituteStmt(SimpleNode nearnode, String str, Map<String, Object> keys) {
...
//   }
  ...
  public static class Profiler {
    ...
    public void pprint() {
      long total = 0;
      for (Block next : current.children) {
        total += next.elapsed;
      }
      for (Block next : current.children) {
  ...
  public void compileBlock(String source) {
    try {
      ...
      cg.setOriginalSource(source);
      String srcDumpFile = (String)options.get(DUMP_SRC_INPUT);
      if (LPS.generateMiddle  && srcDumpFile != null  ) {
        String newname = emitFile(srcDumpFile, source);
        System.err.println("Created " + newname);
      }
      ...
      String astInputFile = (String)options.get(DUMP_AST_INPUT);
      if (LPS.generateMiddle  && astInputFile != null) {
        String newname = emitFile(astInputFile, program);
        System.err.println("Created " + newname);
      }
      // profiler.phase("transform");
      ...
      program = null;
      String astOutputFile = (String)options.get(DUMP_AST_OUTPUT);
      if (LPS.generateMiddle  && astOutputFile != null) {
        String newname = emitFile(astOutputFile, translated);
        System.err.println("Created " + newname);
      }
