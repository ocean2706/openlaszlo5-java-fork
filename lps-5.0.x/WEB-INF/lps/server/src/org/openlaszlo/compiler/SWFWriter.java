...
import org.openlaszlo.media.FontType;
    ...
    public void importBaseLibrary(String library, CompilationEnvironment env) {
        ...
        try {
            ...
            if (LPS.isInternalBuild()) {
                ...
                List<String> newerFiles = new Vector<String>();
                List<File> sourceDirs = new Vector<File>();
                sourceDirs.add(f.getParentFile());
                while (!sourceDirs.isEmpty()) {
                    File ff = sourceDirs.get(0);
                    sourceDirs.remove(0);
                    if (ff.isDirectory()) {
 ...
 */
    ...
    void setCanvas(Canvas canvas, String canvasConstructor) {
        ...
        if (mLogger.isTraceEnabled()) {
          mLogger.trace("    Adding a program of " + action.length + " bytes.");
        }
        addProgram(program);
 ...
 */
                        org.openlaszlo.i18n.LaszloMessages.getMessage(
                                SWFWriter.class.getName(),"051018-410", new Object[] {action.length})
);
         }
         addProgram(program);
 ...
 */
     ...
     */
    public void importResource(List<String> sources, String name, File parent, int frameNum)
    {
        ...
        throws CompilationError
    {
            ...
            if (inputFile.isDirectory()) {
                ...
                ArrayList<String> outsources = new ArrayList<String>();

                for (int i = 0; i < sources.length; i++) {
                    String fname = fileName + File.separator + sources[i];
                    //mLogger.trace("SWFWriter file: " + new File(fname).isFile());

                    if (isFileValidForImport(fname)) {
                        //mLogger.trace("SWFWriter adding: " + fname);
        ...
        if (mLogger.isTraceEnabled()) {
        mLogger.trace("    Importing resource " + name);
        }
        try {
        ...
        Resource res = mResourceMap.get(fileName);
                               ...
                               FontsCollector fontsCollector, boolean addStop)
    {
        ...
        if (mLogger.isTraceEnabled()) {
            mLogger.trace("Including multiple resources as " + name);
        }
        int width = 0;
        int height = 0;
        int fNum = 0;
        for (String fileName : sources) {
            if (mLogger.isTraceEnabled()) {
                mLogger.trace("    Importing " + fileName);
            }
            ...
            if (mLogger.isTraceEnabled()) {
                mLogger.trace("    Added " + (fc - bc) + " of " + fc + "frame(s)");
            }
    ...
    public void openSnippet(String liburl) throws IOException {
        ...
        mFlashFile.getMainScript().getFrameAt(mLastFrame);
 ...
 */
        ...
        if (mLogger.isTraceEnabled() && mProperties.getProperty("trace.fonts", "false").equals("true")) {
            mLogger.trace(actions.toString());
        }
     ...
     private void addResourceTable() {
         ...
         SortedSet<Resource> resources = new TreeSet<Resource>(mMultiFrameResourceSet);
