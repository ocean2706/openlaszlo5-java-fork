 ...
 */
                ...
                if (env.parsedLibraryCache.get(file) != null) {
                  root = env.parsedLibraryCache.get(file);
                } else {
                  ...
                  if (mLogger.isTraceEnabled()) {
                          mLogger.trace("" + file + ": " + root + " attributes: " + root.getAttributes());
                  }
                  ...
                  if (includesAttr != null) {
                     ...
                     Set<File> binaryIncludes = env.getFileResolver().getBinaryIncludes();
                     for (StringTokenizer st = new StringTokenizer(includesAttr);
                       st.hasMoreTokens();) {
                       String name = FileUtils.fromURLPath(st.nextToken());
                       File canon = new File(base, name).getCanonicalFile();
                       if (mLogger.isTraceEnabled()) {
                         mLogger.trace("binary include: " + canon);
                       }
                       visited.add(canon);
                  ...
                  if (keepParsedLibraries != null) {
                      try {
                          ...
                          if (mLogger.isTraceEnabled()) {
                              mLogger.trace("Error saving " + serializedFile + " " + ex);
                          }
                      }
                  }
                }
...
/**
 * @copyright Copyright 2001-2007, 2010, 2011 Laszlo Systems, Inc.  All Rights
